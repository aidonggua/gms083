...
public class MapleClient {
    ...
    public int login(String login, String pwd, String nibbleHwid) {
        ...
        if (loginok == 0 || loginok == 4) {
            AntiMulticlientResult res = MapleSessionCoordinator.getInstance()
                                                               .attemptLoginSession(session, nibbleHwid, accId, loginok == 4);
    ...
    public Calendar getTempBanCalendarFromDB() {
        ...
        try {
            ...
            // 0000-00-00 or 2018-06-20 (default set in LoginPasswordHandler)
            if (blubb == 0 || rs.getString("tempban").equals("2018-06-20 00:00:00.0")) {
                return null;
            }
    ...
    public boolean checkBirthDate(Calendar date) {
        return date.get(Calendar.YEAR) == birthday.get(Calendar.YEAR) && date.get(Calendar.MONTH) == birthday.get(Calendar.MONTH) && date
                .get(Calendar.DAY_OF_MONTH) == birthday.get(Calendar.DAY_OF_MONTH);
    }
        ...
        if (party != null) {
            ...
            if (party.getLeader().getId() == idz && map != null) {
                ...
                for (MaplePartyCharacter pchr : party.getMembers()) {
                    if (pchr != null && pchr.getId() != idz && (lchr == null || lchr.getLevel() <= pchr.getLevel()) && map
                            .getCharacterById(pchr.getId()) != null) {
                        lchr = pchr;
                    }
                }
                if (lchr != null) {
                ...
                if (!(channel == -1 || shutdown)) {
                    if (!cashshop) {
                        if (!this.serverTransition) { // meaning not changing channels
                            ...
                            if (bl != null) {
                                wserv.loggedOff(player.getName(), player.getId(), channel, player.getBuddylist()
                                                                                                 .getBuddyIds());
                            }
                        }
                    } else {
        ...
        for (World w : Server.getInstance().getWorlds()) {
            for (MapleCharacter chr : w.getPlayerStorage().getAllCharacters()) {
                if (accid == chr.getAccountID()) {
                    FilePrinter.print(FilePrinter.EXPLOITS, "Player:  " + chr.getName() + " has been removed from " + GameConstants.WORLD_NAMES[w
                            .getId()] + ". Possible Dupe attempt.");
